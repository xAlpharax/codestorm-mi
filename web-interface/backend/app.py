...
# Add Gemini API imports - fixed import statement
...
if GEMINI_API_KEY:
    try:
        ...
        genai_client = True  # Just a flag to indicate configuration success
...
try:
    ...
    print(f"Upload directory created at {UPLOAD_DIRECTORY}")
except OSError as e:
...
def generate_video_understanding(mode="youtube", task="summarize", file=None, language="en", youtube_url=None):
    """
    Generate understanding of a video using Gemini API.

    Args:
        mode: "youtube" or "video"
        task: "summarize", "transcribe", or "explain"
        file: Path to video file (for mode="video")
        language: Target language code
        youtube_url: YouTube URL (for mode="youtube")
    ...
    # Select prompt based on task and language
    if task == "summarize":
        if language == "en":
    ...
    try:
        ...
        if mode == "youtube" and youtube_url:
            # For YouTube URL - direct approach based on provided examples
            ...
            # For local video file processing
            ...
            # For video file - inline data approach based on provided examples
            generation = model.generate_content([
                {"text": question},
                {
                    "inlineData": {
            ...
            return "Error: Invalid mode or missing required parameters"
        ...
        print(f"Error in generate_video_understanding: {e}")
        return f"Error generating video understanding: {str(e)}"

@app.route('/api/video/youtube', methods=['POST'])
def process_youtube_link():
    """API endpoint to process a YouTube video link."""
    try:
        ...
        # Generate understanding using Gemini API
        response_text = generate_video_understanding(
            mode="youtube",
            task=task,
            language=language,
            youtube_url=url
...
def upload_video_file():
    ...
    try:
        ...
        if not video_file.content_type or not video_file.content_type.startswith('video/'):
            return jsonify({"success": False, "error": "Invalid file type. Only video files are allowed."}), 400
